...
public class OverUnderController {
	...
	public void joinGame(@Header("simpSessionId") String sessionId) throws Exception {
		...
		simpMessagingTemplate.convertAndSendToUser(sessionId, "/queue/joinGame/changes", dto);
	}
	...
	public void gameOver() throws InterruptedException {
		...
		roomOverUnder.getOverTickets().parallelStream().peek(t -> t.setAmountWon(roomOverUnder.getScore())).collect(Collectors.toList());
		roomOverUnder.getUnderTickets().parallelStream().peek(t -> t.setAmountWon(roomOverUnder.getScore())).collect(Collectors.toList());
		...
		roomOverUnder.setCreate(System.currentTimeMillis() + 600 * 1000);
		dto.setCreate(roomOverUnder.getCreate());
		//send start game
		dto.setType(GameEnum.TYPE_MESSAGE.START);
		simpMessagingTemplate.convertAndSend("/overandunder/gameOver", dto);
...
//	}
	...
	@SendToUser
	public void betOver(@Payload Player player, @Header("simpSessionId") String sessionId, Principal principal) throws Exception {
	...
	public void betUnder(@Payload Player player, @Header("simpSessionId") String sessionId) throws Exception {
		...
		if (GameEnum.STATUS.START.equals(roomOverUnder.getStatus())) {
			...
			simpMessagingTemplate.convertAndSendToUser(sessionId, "/queue/betOver/changes", betTicket, headerAccessor.getMessageHeaders());
			simpMessagingTemplate.convertAndSend("/overandunder/gameOver", betTicket);
		}
